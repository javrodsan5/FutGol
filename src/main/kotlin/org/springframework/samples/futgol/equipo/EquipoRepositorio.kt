...
interface EquipoRepositorio : Repository<Equipo, Int> {
    ...
    @Query(value = "SELECT CASE WHEN count(e)> 0 THEN TRUE ELSE FALSE END FROM Equipo e where e.id = ?1 AND e.liga.id = ?2")
    fun existeEquipoEnLiga2(idEquipo: Int, idLiga: Int): Boolean
    ...
    @Query(value = "SELECT CASE WHEN count(e)> 0 THEN TRUE ELSE FALSE END FROM Equipo e where e.liga.id = ?1 AND e.usuario.id= ?2")
